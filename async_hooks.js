...
function callbackTrampoline(asyncId, resource, cb, ...args) {
  ...
  if (asyncId !== 0 && hasHooks(kBefore)) emitBeforeNative(asyncId);
  ...
  if (asyncId !== 0 && hasHooks(kAfter)) emitAfterNative(asyncId);
...
function executionAsyncResource() {
  ...
  const resource =
    execution_async_resources[index] || executionAsyncResource_(index);
...
function trackPromise(promise, parent, silent) {
  ...
  promise[trigger_async_id_symbol] = parent
...
function defaultTriggerAsyncIdScope(triggerAsyncId, block, ...args) {
  if (triggerAsyncId === undefined) return block(...args);
...
function emitBeforeScript(asyncId, triggerAsyncId, resource) {
  ...
  if (hasHooks(kBefore)) emitBeforeNative(asyncId);
}
